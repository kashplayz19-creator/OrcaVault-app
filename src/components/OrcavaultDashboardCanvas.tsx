...
export default function OrcavaultDashboardCanvas({ onReEncrypt }: OrcavaultDashboardCanvasProps) {
  ...
  return (
    <Box className="w-full h-screen border-none rounded-none bg-[#000000] flex flex-col px-8 py-6 overflow-hidden select-none relative orca-scanlines">
      ...
      <Flex className="w-full h-16 mb-5 border-b border-[#1F2226] pb-4 flex-shrink-0" justify="justify-between">
        <Flex gap="gap-6">
          <OrcavaultLogo size={42} className="hover:rotate-12 duration-500" />
          <div className="flex flex-col">
            <h1 className="font-mono text-xl text-white tracking-[0.25em] font-bold">ORCA<span className="text-[#00F0FF] opacity-90">VAULT</span></h1>
            <span className="font-mono text-[9px] text-[#00F0FF] opacity-60 uppercase tracking-[0.4em]">Decentralized sovereign asset terminal</span>
          </div>
        </Flex>
        ...
        <div className="flex-1 max-w-xl mx-8">
          <form onSubmit={handleSearch} className="flex gap-2">
            ...
            <button 
              type="submit" 
              className="px-6 py-2 bg-[#00F0FF]/10 text-[#00F0FF] border border-[#00F0FF]/30 hover:bg-[#00F0FF]/25 rounded text-xs font-mono font-bold tracking-widest active:scale-[0.98] cursor-pointer transition-all duration-300"
            >
              QUERY
            </button>
          </form>
        ...
        <Flex gap="gap-6" className="font-mono text-[10px] text-slate-400">
          ...
          {onReEncrypt && (
            <button 
              onClick={onReEncrypt}
              className="px-3.5 py-1.5 border border-red-500/20 bg-[#0B0C0E] text-red-400 hover:bg-red-500/10 hover:text-red-300 rounded text-[9px] font-bold tracking-widest active:scale-[0.98] flex gap-1.5 items-center cursor-pointer transition-all duration-300"
            >
      ...
      <Flex className="w-full h-8 mb-4 flex-shrink-0 font-mono text-[10px] text-slate-500 border-b border-[#1F2226]/40 pb-2" justify="justify-start" gap="gap-3">
        ...
        {cache.map((item, index) => (
          <button
            ...
            className={`px-3 py-1 border rounded active:scale-[0.98] transition-all uppercase cursor-pointer ${currentTicker === item ? 'bg-[#00F0FF]/10 text-[#00F0FF] border-[#00F0FF]/45 font-bold shadow-[0_0_8px_rgba(0,240,255,0.15)]' : 'bg-[#0B0C0E] border-[#1F2226] text-slate-400 hover:text-slate-200'}`}
          >
            {item}
          </button>
        ))}
        ...
        <Stack className="flex-1 overflow-y-auto pr-2 scrollbar-thin" gap="gap-5">
          ...
          <Grid cols="grid-cols-12" gap="gap-5" className="flex-none">
            ...
            <Box className="col-span-5 p-5 border-[#1F2226] bg-[#0B0C0E] flex flex-col justify-between shadow-[0_4px_24px_rgba(0,0,0,0.8)] relative group overflow-hidden">
              ...
              <Stack gap="gap-2.5" className="mt-4 font-mono">
                <Flex className={`px-4 py-2 rounded border ${getGradeBg(metrics.profitabilityGrade)}`} justify="justify-between">
                  <span className="text-[10px] text-slate-400 tracking-wider">OPERATING PROFITABILITY</span>
                  <span className={`text-sm font-bold ${getGradeColor(metrics.profitabilityGrade)}`}>{metrics.profitabilityGrade}</span>
                </Flex>
                <Flex className={`px-4 py-2 rounded border ${getGradeBg(metrics.valuationGrade)}`} justify="justify-between">
                  <span className="text-[10px] text-slate-400 tracking-wider">VALUATION INTEGRITY</span>
                  <span className={`text-sm font-bold ${getGradeColor(metrics.valuationGrade)}`}>{metrics.valuationGrade}</span>
                </Flex>
                <Flex className="px-4 py-2 rounded border bg-[#0B0C0E] border-[#1F2226]" justify="justify-between">
                  <span className="text-[10px] text-slate-400 tracking-wider">REVENUE ASSET GROWTH</span>
                  <span className="text-sm font-bold text-[#00F0FF]">{metrics.growthGrade}</span>
                </Flex>
                <Flex className="px-4 py-3 bg-[#00F0FF]/5 rounded border border-[#00F0FF]/30 mt-2 shadow-[0_0_12px_rgba(0,240,255,0.05)]" justify="justify-between">
                  <span className="text-xs text-white font-bold tracking-widest">ORCA VAULT RATING</span>
                  <span className="text-2xl font-bold text-[#00F0FF] drop-shadow-[0_0_8px_rgba(0,240,255,0.6)]">{metrics.overallGrade}</span>
                </Flex>
              </Stack>
            </Box>
          ...
          <Grid cols="grid-cols-12" gap="gap-5" className="flex-1 min-h-0 font-mono">
            ...
            <Box className="col-span-7 p-5 border-[#1F2226] bg-[#0B0C0E] flex flex-col shadow-[0_4px_24px_rgba(0,0,0,0.8)] relative min-h-[220px]">
              <Flex justify="justify-between" className="border-b border-[#1F2226] pb-3 mb-3 flex-shrink-0">
                <span className="text-xs text-white font-bold tracking-widest flex items-center gap-2">
                  <Database className="w-3.5 h-3.5 text-slate-400" />
                  PEER-MATRIX DATASHEET
                </span>
                <span className="text-[9px] text-[#00F0FF] uppercase">SECTOR_ADJACENT SEC_DATA</span>
              </Flex>
              ...
              <div className="overflow-y-auto flex-1 pr-1 text-[10px]">
                <table className="w-full text-left">
                  ...
                  <tbody className="divide-y divide-[#1F2226]/50">
                    ...
                    <tr className="hover:bg-[#1C1E22]/30 text-slate-400 active:scale-[0.98] transition-all cursor-pointer">
            ...
            <Box className="col-span-5 p-5 border-[#1F2226] bg-[#0B0C0E] flex flex-col shadow-[0_4px_24px_rgba(0,0,0,0.8)] relative">
              <Flex justify="justify-between" className="border-b border-[#1F2226] pb-3 mb-3 flex-shrink-0">
                ...
                <span className="text-[8px] text-slate-500">STITCH WATCHLOCK</span>
              </Flex>
